...
def main(config):
    ...
    # 数据模块
    data_loader = config.init_obj('data_loader', module_data) #训练集dataloader
    valid_data_loader = data_loader.split_validation()#验证集dataloader

    # 模型模块
    #剪枝优化
    refine = config['model_jianzhi']['refine']
    print(refine)
    if refine:
      checkpoint = torch.load(refine)
      print(checkpoint['cfg'])
      model = ASPNET(net_name=checkpoint['cfg'])
      model.cuda(config['n_gpu'])
    ...
    #迁移学习
    #checkpoint = torch.load('./saved/models/SPOOF-NET/0505_021119/checkpoint-epoch200.pth')
    #model.load_state_dict(checkpoint['state_dict'])
...
if __name__ == '__main__':
    ...
    copyfile_config = "%s/config.json" % (config.save_dir)
    copyfile('config.json', copyfile_config)
    main(config)
